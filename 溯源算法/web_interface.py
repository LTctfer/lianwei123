...
from gaussian_plume_model import PollutionSource, MeteoData, GaussianPlumeModel
...
class WebInterface:
    ...
    def _result_visualization(self):
        ...
        viz_option = st.selectbox(
            "选择可视化类型",
            ["算法性能对比", "浓度场分布", "收敛过程", "传感器分布", "误差分析", "正向验证"]
        )
        ...
        elif viz_option == "正向验证":
            self._show_forward_validation(analysis_data)
    ...
    def _show_forward_validation(self, analysis_data):
        """正向传播验算：用反算的源正向计算浓度，与观测对比给出准确度"""
        meteo_data = analysis_data['meteo_data']
        sensor_data = analysis_data['sensor_data']
        results = analysis_data['results']

        # 选择一个算法结果
        selected_alg = st.selectbox("选择算法进行正向验证", list(results.keys()))
        res = results[selected_alg]

        # 构造反算得到的污染源
        est_source = PollutionSource(x=res.source_x, y=res.source_y, z=res.source_z, emission_rate=res.emission_rate)
        model = GaussianPlumeModel()

        # 计算每个传感器的预测浓度
        pred, obs, sid = [], [], []
        for s in sensor_data:
            c_pred = model.calculate_concentration(est_source, s.x, s.y, s.z, meteo_data)
            pred.append(c_pred)
            obs.append(s.concentration)
            sid.append(s.sensor_id)
        ...
        fig = px.scatter(
            df, x='obs', y='pred', hover_name='sensor_id',
            labels={'obs': '观测浓度 (μg/m³)', 'pred': '预测浓度 (μg/m³)'},
            title=f"正向验证: {selected_alg} 结果"
        )
        fig.add_shape(type='line', x0=df['obs'].min(), y0=df['obs'].min(),
                      x1=df['obs'].max(), y1=df['obs'].max(), line=dict(color='gray', dash='dash'))
        st.plotly_chart(fig, use_container_width=True)
    ...
    def _generate_comprehensive_report(self, analysis_data=None):
        """生成综合分析报告"""
        st.info("综合分析报告已生成")

    def _generate_performance_report(self, analysis_data=None):
        """生成算法性能报告"""
        st.info("算法性能报告已生成")

    def _generate_visualization_report(self, analysis_data=None):
        """生成可视化报告"""
        st.info("可视化报告已生成")
